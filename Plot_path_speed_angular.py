...
all_data = []
for i in range(3):
    filename = folder_path+'iris_angular_vel'+str(i)+'.txt'
    data_list = np.loadtxt(filename)
    data_array = np.array(data_list)
    all_data.append(data_array)

# Stack the data from all files
iris_angular_vel_data = np.array(all_data).T

size = iris_angular_vel_data.shape
num_rows, num_columns = size
print("angular velocity Number of rows:", num_rows)
print("angular velocity Number of columns:", num_columns)

iris_angular_vel_x = iris_angular_vel_data[:, 0]
iris_angular_vel_y = iris_angular_vel_data[:, 1]
iris_angular_vel_z = iris_angular_vel_data[:, 2]
#######################################3
...
plt.plot(x, iris_angular_vel_x[crop_data:]*radian_to_degree, marker='', linestyle='-', color='g', label='angular x')
plt.plot(x, iris_angular_vel_y[crop_data:]*radian_to_degree, marker='', linestyle='-', color='m', label='angular y')
plt.plot(x, iris_angular_vel_z[crop_data:]*radian_to_degree, marker='', linestyle='-', color='y', label='angular z')

abs_angular_vel_x = [abs(num) for num in iris_angular_vel_x]
abs_angular_vel_y = [abs(num) for num in iris_angular_vel_y]
...
print("abs_angular_vel_x:", abs_angular_vel_x)  # 0.15
print("abs_angular_vel_y:", abs_angular_vel_y)  # 0.31
# Set labels for the axis
plt.xlabel('Time [second]')
